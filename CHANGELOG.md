...
with an icon, a README and nothing to see.

Both shots are real Dart in the real editor, one per variant, generated by
`tool/ds_retro_shots.py` in the portfolio repo — it re-fetches the theme JSON
from GitHub on each run, so the images follow a colour change rather than
